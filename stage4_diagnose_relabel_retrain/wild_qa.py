"""Re-run the same 30 Stage 3 wild-QA samples (../stage3_first_model_run/
data/wild_samples.json -- reused, not re-sourced, so results are directly
comparable) against the Stage 4 model, with the pattern-2 adjacent-merge
postprocess applied, to check which of the original 10 problem patterns
improved/regressed/unresolved.
"""
import json
import os
...
STAGE3_SAMPLES_PATH = os.path.join(HERE, "..", "stage3_first_model_run", "data", "wild_samples.json")
...
def tag_text(text, tokenizer, model):
    ...
    spans = merge_adjacent_same_label(text, spans)
...
def main():
    ...
    with open(STAGE3_SAMPLES_PATH, encoding="utf-8") as f:
        samples = json.load(f)
